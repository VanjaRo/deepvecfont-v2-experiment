...
def train_main_model(opts):
    ...
    if opts.last_ckpt != "none":
        ...
        model_main.load_state_dict(ckpt["model"])
    ...
    for epoch in range(epochs_passed, opts.n_epochs):
        for idx, data in enumerate(train_loader):
            ...
            time_loss_graph_info.append((time(), loss.item()))
    ...
    with open(f"loss_time_{int(time())}.pkl", "wb") as f:
        pickle.dump(time_loss_graph_info, f)
...
def main():
    ...
    # 50_16371.ckpt
    # opts.add_argument("--last_ckpt", type=str, default="none")
