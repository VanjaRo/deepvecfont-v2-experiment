        ...
        for i in range(
            ...
            for char_id in range(num_chars):
                ...
                if not os.path.exists(
                    os.path.join(
                        cur_font_sfd_dir,
                        "roman",
                        f"{font_id}_{char_id}:0{num_chars_w}.sfd",
                    )
                ):
                    break
                ...
                char_desp_f = open(
                    os.path.join(
                        cur_font_sfd_dir,
                        "roman",
                        f"{font_id}_{char_id}:0{num_chars_w}.txt",
                    ),
                    "r",
                )
            ...
            if opts.split == "train":
                ...
                for char_id in range(num_chars):
                    ...
                    cur_italic_glyphs.append(example)
                    char_desp_f.close()
                    sfd_f.close()
            ...
            if len(cur_font_glyphs) == num_chars:
                ...
                rendered = np.load(
                    os.path.join(
                        cur_font_sfd_dir, "roman", "imgs_" + str(opts.img_size) + ".npy"
                    )
                )
                if opts.split == "train":
                    rendered_italic = np.load(
                        os.path.join(
                            cur_font_sfd_dir,
                            "italic",
                ...
                for char_id in range(num_chars):
                    ...
                    if opts.split == "train":
                        example_italic = cur_italic_glyphs[char_id]
                        sequence_italic.append(example_italic["sequence"])
                        seq_len_italic.append(example_italic["seq_len"])
                        char_class_italic.append(example_italic["class"])

                if not os.path.exists(os.path.join(output_path, f"{i}:0{num_fonts_w}")):
                    os.mkdir(os.path.join(output_path, f"{i}:0{num_fonts_w}"))
                ...
                if opts.split == "train":
                    ...
                    np.save(
                        ...
                        np.array(char_class_italic),
                    )
                    np.save(
                        os.path.join(
                            output_path,
                            f"{i}:0{num_fonts_w}",
                            "rendered_" + str(opts.img_size) + "_italic.npy",
                        ),
                        rendered_italic,
                    )
...
def main():
    ...
    # default is "../data/vecfont_dataset_/eng/train"
