        ...
        if mode == "train":
            ref_cls = torch.tensor(
                np.array(
                    [
                        self.rng.choice(
                            self.opts.char_num, self.opts.ref_nshot, replace=False
                        )
                        for _ in range(input_image.size(0))
                    ]
                )
            ).cuda()
            # ref_cls = torch.randperm() randint(0, self.opts.char_num, (input_image.size(0), self.opts.ref_nshot)).cuda()
        elif mode == "val":
            ref_cls = (
